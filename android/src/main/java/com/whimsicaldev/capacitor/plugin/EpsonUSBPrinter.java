...
public class EpsonUSBPrinter {
    ...
    private boolean isAPrinter(UsbDevice usbDevice) {
        for (int i = 0; i < usbDevice.getInterfaceCount(); i += 1) {
            ...
            for (int j = 0; j < usbInterface.getEndpointCount(); j++) {
                ...
                return (
                    UsbConstants.USB_ENDPOINT_XFER_BULK == usbEndpoint.getType() && UsbConstants.USB_DIR_OUT == usbEndpoint.getDirection()
                );
            }
        }
        return false;
    ...
    public void print(String printObject, int lineFeed, String charset) throws Exception {
        ...
        for (EpsonUSBPrinterLineEntry lineEntry : printObjectList) {
            ...
            if (lineEntry.getLineText() != null) {
                ...
                for (String print : splitData) {
                    //        this.connection.bulkTransfer(this.usbEndpoint, print.getBytes("gbk"), print.getBytes("gbk").length, 10000);
                    this.connection.bulkTransfer(this.usbEndpoint, print.getBytes(charset), print.getBytes(charset).length, 10000);
